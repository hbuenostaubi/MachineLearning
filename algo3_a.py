...
# -*- coding: utf-8 -*-
"""
Created on Sun Jul 26 01:50:01 2020
...
def main():    
    ...
    #Find the predicted rating for user u on item i   
    movieTest = predict_rating(movieTest, movielens_itemDev,movielens_userDev,movielens_global_mean)
    
    #Calculate the MAE 
    movie_mae=mean_absolute_error(movieTest)
    print('MAE baseline model (3a):', movie_mae)
    ...
    #Find the predicted rating for user u on item i
    yelpTest= predict_rating(yelpTest, yelp_itemDev,yelp_userDev,yelp_global_mean)
    
    #Calculate the MAE 
    yelp_MAE = mean_absolute_error(yelpTest)
    print('MAE of yelp dataset:', yelp_MAE)
...
len(lst)
plt.plot(lst, "b.")
plt.ylabel("Loss Function (MovieLens)")
plt.xlabel("Number of Epochs")

len(lst2)
plt.plot(lst2, "b.")
plt.ylabel("Loss Function (Yelp)")
plt.xlabel("Number of Epochs")
